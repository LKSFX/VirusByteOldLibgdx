package com.lksfx.virusByte.gameControl.swiper.simplify;

import com.badlogic.gdx.math.Vector2;
import com.badlogic.gdx.utils.Array;
import com.lksfx.virusByte.gameControl.swiper.SwipeResolver;

public class ResolverRadialChalkin implements SwipeResolver {
	...
	public void resolve(Array<Vector2> input, Array<Vector2> output) {
		output.clear();
		if ( input.size <= 2 ) { // simply copy
			output.addAll( input );
			return;
		}
		
		//Simply with squared tolerance
	...
	public static void smooth(Array<Vector2> input, Array<Vector2> output) {
		...
		for (int i = 0; i < input.size-1; i++) {
			...
			Vector2 Q = new Vector2(.75f * p0.x + .25f * p1.x, .75f * p0.y + .25f * p1.y);
			Vector2 R = new Vector2(.25f * p0.x + .75f * p1.x, .25f * p0.y + .75f * p1.y);
			output.add(Q);
			output.add(R);
		}
	...
	/**Simply distance-based simplification*/
